...
Summary quality was quantified with twelve automatic metrics spanning three dimensions: lexical overlap, semantic similarity, and factual consistency (see ``Evaluation metrics'' in STAR Methods).
...
Based on overall performance ranks, referred to as ``Performance Rank" and derived from our multi-metric evaluation framework (see ``Overall Performance Score'' in STAR Methods), models from the Mistral family were top-ranked across the three evaluated metric dimensions, as depicted in Figure~\ref{fig:rank_heatmap}. 
...
To rule out that benchmark articles were present in the models' training data, we applied a two-step assessment: first quantifying each model's potential overlap, defined as the share of benchmark articles published on or before its training cutoff date, and then probing the models with high overlap for actual memorization (see ``Data leakage analysis'' in STAR Methods).
...
Eight blinded domain experts rated summaries from four models along coherence, fluency, relevance, and consistency on a 1–5 scale (see ``Expert assessment'' in STAR Methods).
...
A panel of three provider-independent judge models scored the same four dimensions, with the median across judges taken as the panel score (see ``LLM-as-a-judge evaluation'' in STAR Methods).
