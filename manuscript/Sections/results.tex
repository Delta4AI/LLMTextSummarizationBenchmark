...
Gwet's AC2 (quadratic weights) and mean pairwise Spearman's $\rho$ measure inter-rater agreement.}
...
\begin{table}[ht]
\centering
\caption{LLM-as-a-judge panel vs.\ expert assessments (Spearman's $\rho$, $n=80$
summaries). \emph{Diagonal}: judge dimension vs.\ the same expert dimension.
\emph{Leave-one-out ceiling}: each expert vs.\ the mean of the others (fair, same-target
human upper bound). \emph{Pairwise}: raw inter-rater agreement between two
individual experts.}
\label{tab:judge-expert}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[ht]
\centering
\caption{System-level rank concordance between the LLM-as-a-judge ranks and each evaluation metric category across all $n=62$ models. Judge ranks derive from the 20-publication subset, metric ranks use the full benchmark. This measures agreement between two automatic methods, not human validation.}
...
\begin{table}[ht]
\centering
\caption{LLM-as-a-judge self-preference: a judge's mean score for its own model family minus its score for other systems, each measured relative to the other two judges (1-5 scale). Positive values indicate inflation of the judge's own family.}
\label{tab:selfpref}
\begin{tabular}{lc}
\toprule
