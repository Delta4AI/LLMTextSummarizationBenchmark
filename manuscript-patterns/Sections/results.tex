...
We evaluated 62 summarization models on 1,000 biomedical abstracts, using author-provided highlights as reference summaries. All models were prompted uniformly to generate 15--100 word summaries. Evaluation combined lexical overlap metrics (ROUGE-1/2/L, BLEU, METEOR), semantic similarity metrics (RoBERTa, DeBERTa, all-mpnet-base-v2), and a factual consistency metric (AlignScore). An Overall Performance Score was computed as a weighted combination of the three metric dimensions (see Methods, Section~\ref{subsec:overall_performance_metric}).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Visualizations/metric_correlation.png}
\caption{Correlation matrix of evaluation metrics. Spearman correlation coefficients ($\rho$) between two metrics based on their mean scores across all models are given. Metric categories (lexical, semantic, factual) are indicated on the x and y axes. For visualization purposes, cells with values higher than 0.8 are shown with white text.\label{fig:metric_correlation}}
\end{figure}

\subsection{Overall Model Performance}

Based on overall performance ranks, referred to as "Performance Rank" and derived from our multi-metric evaluation framework (see Methods, Section~\ref{subsec:overall_performance_metric}), models from the Mistral family were top-ranked with high performance scores across the three evaluated metric dimensions, as depicted in Figure~\ref{fig:rank_heatmap}. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Visualizations/category_boxplot.png}
\vspace{8pt}
\includegraphics[width=0.8\textwidth]{Visualizations/category_gameshowell.png}
\caption{
\textbf{(a)} Distributions of overall performance scores across the nine model categories,
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Visualizations/family_boxplot.png}
\vspace{8pt}
\includegraphics[width=0.8\textwidth]{Visualizations/family_gameshowell.png}
\caption{
\textbf{a)} Distributions of overall performance scores across all model families,
...
We selected \textbf{mistral-small3.2:24b} as a high-performing model (ranked 2nd overall) and \textbf{medllama2:7b} as a substantially lower-ranked model (ranked 54th out of 62) to contrast differences in concept coverage. The generated summaries are as follows:
...
Table~\ref{tab:concept_coverage} summarizes the concept-level coverage and overall performance for both models. While both models capture the switch-like behavior (H1) and the percolation transition (H3), the lower-performing model fails to explicitly link the threshold to surface density/spacing (H2) and omits the complexity-based decision-making framework (H4). In contrast, \textbf{mistral-small3.2:24b} achieves full coverage by explicitly connecting the physical density to the "decisions" made by the complex protein network. The overall performance scores, derived using the weighted metric aggregation defined in the Methods (Section~\ref{subsec:overall_performance_metric}), further reflect this difference, with \textbf{mistral-small3.2:24b} achieving a higher score compared to \textbf{medllama2:7b}.

\begin{table}[H]
\centering
\caption{Concept coverage analysis of model-generated summaries. Symbols: \checkmark = fully covered; $\sim$ = partially covered; $\times$ = not covered. Overall performance scores are derived using the weighted metric aggregation defined in the Methods (Section~\ref{subsec:overall_performance_metric}).}
